...
def get_morphology_dict(token: Token) -> Dict:
    ...
    if "|" not in morphology:
        return {}

    morphology_dict = dict([prop.split("=") for prop in morphology.split("|")])
    return morphology_dict

def get_feature_dict(word) -> Dict:
    features = str(word.feats)
    if "|" not in features:
        return {}

    features_dict = dict([prop.split("=") for prop in features.split("|")])
    return features_dict

def get_gender_from_word_stanza(sent):
    feats_dict = get_feature_dict(sent)
    if "Gender" not in feats_dict:
        return None

    gender = SPACY_GENDER_TYPES[feats_dict["Gender"]]
    return gender
